...
This update introduces a robust management layer for LLM (Large Language Model) interactions within the VitalisXBackend. The primary goals were to:
...
- **Pre-invocation Checks**: Quota and rate-limit checks occur _before_ any LLM call to avoid incurring costs on rejected requests.
